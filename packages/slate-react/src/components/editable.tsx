...
import { Editor, Element, NodeEntry, Node, Range, Text, Transforms, Path } from '@solidoc/slate';
        ...
        onPaste={useCallback(
          (event: React.ClipboardEvent<HTMLDivElement>) => {
            ...
            if (
              ...
              (!HAS_BEFORE_INPUT_SUPPORT || isPlainTextOnlyPaste(event.nativeEvent)) &&
              !readOnly
            ) {
              event.preventDefault();
...
const hasEditableTarget = (editor: ReactEditor, target: EventTarget | null): target is DOMNode => {
...
const isEventHandled = <EventType extends React.SyntheticEvent<unknown, unknown>>(
  event: EventType,
